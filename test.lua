...
-- ========================================
-- FlurioreLib_BW (đen trắng) + shim Tabs.*
-- https://raw.githubusercontent.com/nyannos/Ui-Script/refs/heads/main/FlurioreLib_BW.lua
print("[nyann os] loading UiREDzV2...")

-- Load Redz V2 (global API: MakeWindow, MakeTab, AddToggle, ...)
pcall(function()
  loadstring(game:HttpGet(("https://raw.githubusercontent.com/daucobonhi/Ui-Redz-V2/refs/heads/main/UiREDzV2.lua")))()
end)

if type(MakeWindow) ~= "function" then
  error("[nyann os] UiREDzV2 failed to load (MakeWindow missing)")
end
...
MakeWindow({
  ...
  Key = {
    ...
    Title = "",
    Description = "",
    Keys = { "nyann-premium-7461b46feacae25298f3f7fd8806196aBV" },
    KeyLink = "",
    Notifi = {
      Notifications = true,
      Incorrectkey = "√",
...
-- Shim Tabs.* → Redz MakeTab / AddToggle / AddButton / ...
local function MakeTabShim(parentTab)
  local tab = {}

  function tab:AddSection(name)
    -- Redz V2 không có section riêng; dùng label/button giả
    if type(name) == "table" then name = name[1] or name.Name or "Section" end
    name = tostring(name or "Section")
    pcall(function()
      if type(AddButton) == "function" then
        AddButton(parentTab, {
          Name = "— " .. name .. " —",
  ...
  function tab:AddDropdown(cfg)
    ...
    local cb = cfg.Callback or function() end
    -- Redz may have AddDropdown; fallback to buttons
    local ok = false
    pcall(function()
      if type(AddDropdown) == "function" then
        AddDropdown(parentTab, {
          Name = name,
          Options = opts,
          Default = def,
          Callback = cb,
    ...
    if not ok then
      pcall(function()
        AddButton(parentTab, {
          Name = name .. " (dropdown fallback)",
          Callback = function()
            if type(opts) == "table" and opts[1] then cb(opts[1]) end
          end,
        })
      end)
    end
  ...
  function tab:AddSlider(cfg)
    ...
    local cb = cfg.Callback or function() end
    pcall(function()
      if type(AddSlider) == "function" then
        AddSlider(parentTab, {
          Name = name,
          Min = minv,
          Max = maxv,
  ...
  function tab:AddTextBox(cfg)
    ...
    local cb = cfg.Callback or function() end
    pcall(function()
      if type(AddTextBox) == "function" then
        AddTextBox(parentTab, {
          Name = name,
          Default = cfg.Default or cfg.Placeholder or "",
          Callback = cb,
        })
      elseif type(AddButton) == "function" then
        AddButton(parentTab, { Name = name, Callback = function() cb("") end })
      end
    end)
  end
  ...
  function tab:AddDiscordInvite(cfg)
    ...
    pcall(function()
      AddButton(parentTab, {
        Name = tostring(cfg.Title or "Discord"),
        Callback = function()
          if cfg.Invite then
            pcall(function() setclipboard(tostring(cfg.Invite)) end)
          end
        end,
      })
    end)
...
function Window:MakeTab(cfg)
  ...
  local ok, res = pcall(function()
    return MakeTab({ Name = title })
  end)
  if ok then page = res end
  if not page then
    warn("[nyann os] MakeTab fail:", title, res)
    page = nil
  end
...
function Window:Notify(opts)
  ...
  pcall(function()
    if type(MakeNotifi) == "function" then
      MakeNotifi({
        Title = title,
        Text = msg,
        Time = opts.Duration or 4,
      })
      return
    end
  end)
