...
do
  ...
  pcall(function() vim1 = game:GetService("VirtualInputManager") end)
  pcall(function() vim2 = game:GetService("VirtualUser") end)
  TeamSelf = plr.Team
...
-- ========================================
print("[nyann os] loading Xezios UI...")
local Library
do
  local urls = {
    "https://raw.githubusercontent.com/i77lhm/Libraries/refs/heads/main/Xezios/Library.lua",
  }
  local lastErr
  for _, url in ipairs(urls) do
    local ok, res = pcall(function()
      local src = game:HttpGet(url)
      if type(src) ~= "string" or #src < 1000 then error("empty lib") end
      local fn, err = loadstring(src)
      if not fn then error(err or "compile fail") end
      fn() -- Xezios sets getgenv().Library
      return getgenv().Library
    end)
    if ok and res then
      Library = res
      print("[nyann os] Xezios OK")
      break
    else
      lastErr = res
      warn("[nyann os] Xezios fail", res)
    end
  end
  if not Library then
    error("[nyann os] Cannot load Xezios Library: " .. tostring(lastErr))
  end
end
...
local RealWindow = Library:Window({
  Prefix = "nyann os by real",
  Suffix = "_@nyannnokonoko",
  Size = UDim2.fromOffset(800, 580), -- taller so all tabs fit
})

pcall(function()
  if RealWindow and RealWindow.ChangeMenuTitle then
    RealWindow.ChangeMenuTitle("nyann os by real _@nyannnokonoko")
  elseif RealWindow and RealWindow.Items and RealWindow.Items.UITitle then
    RealWindow.Items.UITitle.Text = "nyann os by real _@nyannnokonoko"
  end
end)
...
local function nextSide()
  local s = rawget(_G, "__xezios_side") or "Left"
  _G.__xezios_side = (s == "Left") and "Right" or "Left"
  return s
end
...
local function MakeTabShim(page)
  ...
  pcall(function()
    section = page:Section({ Name = "Main", Side = "Left" })
  end)
  ...
  function tab:AddSection(name, forceSide)
    ...
    if ok and s then
      ...
      ok, s = pcall(function() return page:Section({ Name = name }) end)
      if ok and s then section = s end
    end
    return section
  end
  ...
  function tab:AddToggle(cfg)
    ...
    pcall(function()
      ret = sec:Toggle({
        ...
        Callback = cb,
        callback = cb, -- Xezios uses lowercase key internally
      })
    end)
    return ret
  end
  ...
  function tab:AddParagraph(a, b)
    ...
    local full = title
    if desc ~= "" then full = title .. " | " .. desc end
    local labelObj
    pcall(function()
      labelObj = sec:Label({ Name = full })
    end)
    local proxy = {
      _text = full,
      Set = function(self, t)
        ...
        pcall(function()
          if labelObj and labelObj.Items and labelObj.Items.Title then
            labelObj.Items.Title.Text = self._text
          end
        end)
      end,
  ...
  function tab:AddTextBox(cfg)
    ...
    pcall(function()
      ret = sec:Textbox({
        ...
        Default = cfg.Default or cfg.Value or "",
        PlaceHolder = cfg.Placeholder or cfg.PlaceholderText or "",
        Callback = cfg.Callback or function() end,
        Flag = cfg.Flag or cfg.Name,
      })
    end)
  ...
  function tab:AddSlider(cfg)
    ...
    pcall(function()
      ret = sec:Slider({
        ...
        Min = cfg.Min or 0,
        Max = cfg.Max or 100,
        Default = cfg.Default or cfg.Value or 0,
  ...
  function tab:AddDiscordInvite(cfg)
    ...
    local sec = ensureSection()
    pcall(function()
      sec:Label({ Name = tostring(cfg.Title or cfg.Name or "Discord") })
      if cfg.Description then sec:Label({ Name = tostring(cfg.Description) }) end
      if cfg.Invite then sec:Label({ Name = tostring(cfg.Invite) }) end
    end)
  end
...
function Window:MakeTab(cfg)
  cfg = cfg or {}
  local title = cfg.Title or cfg.Name or cfg[1] or "Tab"
  local icon = cfg.Icon or cfg[2] or "rbxassetid://112730572155522"
  -- Xezios uses :Tab not :Page
  local page
  local ok, res = pcall(function()
    return RealWindow:Tab({
      Name = tostring(title),
      name = tostring(title),
      Icon = icon,
      icon = icon,
  ...
  if not page then
    ...
    return MakeTabShim({ Section = function() return nil end })
  end
...
function Window:NewMinimizer()
  return { CreateMobileMinimizer = function() end }
end
function Window:SetUIScale() end
...
local Tabs = {
    ...
    Music = Window:MakeTab({ Title = "Music", Icon = "rbxassetid://" }),
  ...
  if _G.MuteGame then
    ...
    pcall(function()
      for _, s in ipairs(workspace:GetDescendants()) do
        if s:IsA("Sound") and s ~= MusicSound then
          s.Volume = 0
        end
      end
    end)
    ...
    pcall(function() ss.Volume = vol end)
  end
...
Tabs.Music:AddToggle({
  ...
  Description = "Silence in-game audio (keep music)",
  Default = false,
  Callback = function(v)
    _G.MuteGame = v
    ApplyGameVolume()
...
-- Keep mute applied (some games recreate sounds)
task.spawn(function()
  while task.wait(2) do
    if _G.MuteGame then
      pcall(ApplyGameVolume)
    end
    -- ensure music volume stays set
    pcall(function()
      if _G.MusicPlaying and MusicSound then
        MusicSound.Volume = tonumber(_G.MusicVolume) or 0.6
      end
    end)
  end
