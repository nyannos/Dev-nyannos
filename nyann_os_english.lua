...
local function NotificacaoNightMystic(titulo, mensagem)
    local success = pcall(function()
        ...
        local LogoID = "rbxassetid://94678517792779"
    ...
    if not success then
        warn("[nyann os] Error displaying notification")
    end
end

NotificacaoNightMystic("nyann os", "Script loaded successfully!")

-- ========================================
-- SAVE SYSTEM (Optimized)
-- ========================================
local HttpService = Services.HttpService
local FolderName = "nYaNn"
...
G.Kill = function(I, e)
	...
	-- TP above the mob (unique height)
...
_G.MaxBringMobs = _G.MaxBringMobs or 3 -- MOB LIMIT
...
--==================================================
-- FUNCTION: CHECKS IF ANY FARM IS ACTIVE
--==================================================
local function FarmAtivo()
    -- ABSOLUTE PRIORITY (ELF)
    if _G.FarmPriorityElf or _G.FarmElfLevelCustom then
        return true
    end

    -- AUTO MASTERY ALL SWORD (INDEPENDENT OF START FARM)  
    if _G.FarmMastery_S then  
        return true  
    end  

    -- OTHER FARMS (DEPENDENT ON START FARM)  
...
--==================================================
-- FUNCTION: IGNORES UNWANTED MOBS
--==================================================
...
-- Optimize FPS, clean RAM, remove effects, reduce lag comprehensively
-- ============================================================
...
-- Remove post-processing effects in Lighting
local function CleanLightingEffects()
    ...
    -- Keep Sky (sun/moon) but remove FogAtmosphere if any
    for _, child in ipairs(Lighting:GetChildren()) do
...
-- Disable particles, fire, smoke, sparkles effects on NPC/Enemies
...
-- Reduce render quality to lowest
...
-- Clean RAM: remove unnecessary objects
local function CleanRAM()
    -- Remove Rocks if still present
    pcall(function()
        local rocks = workspace:FindFirstChild("Rocks")
        if rocks then rocks:Destroy() end
    end)
    -- Remove dead enemies (Humanoid.Health <= 0)
    pcall(function()
...
-- Disable shadow for all BasePart in Map (chunked to avoid lag)
...
-- Disable NPC animation (reduce CPU)
...
LowCpu = function()
    ...
    -- Auto clean RAM every 30 seconds when optimizer is on
    if not _G.AutoRAMClean then
        ...
        task.spawn(function()
            while _G.OptimizerActive do
                task.wait(30)
                CleanRAM()
                -- Re-disable particles on newly spawned NPCs
                task.spawn(CleanWorkspaceEffects)
            end
            _G.AutoRAMClean = false
        end)
    ...
    -- Hook DescendantAdded to disable effects on newly spawned NPCs
...
-- [[ VARIABLES FOR YOUR INPUT ]] --
getgenv().TweenSpeedFar = 300   -- Default Speed (Far)
getgenv().TweenSpeedNear = 600  -- Boost Speed (Near <= 15 studs)
...
QuestCheck = function()
    ...
    -- [FIX] Fix level within the mission limit
    if World1 and I > 699 then
        I = 650 -- Force the Galley Captain mission (Lv 650)
    end
    
    if World2 and I > 1499 then
        I = 1450 -- Force the Water Fighter mission (Lv 1450)
    end
...
MaterialMon = function()
    ...
    -- Checks if you are far from the entrance. If so, requests to enter.
...
local Library = loadstring(game:HttpGet("https://raw.githubusercontent.com/id202azh/id_905/refs/heads/main/raw_3_white.txt"))():MakeWindow({
    Title = "nyann os",
    SubTitle = "by real_@nyannn",
...
-- Variable to store the Level Farm mob
local CurrentMobLevel = nil
local CurrentMobLevelSystem = nil

-- Defines the distance limit for conditional teleport (in studs)
local TELEPORT_DISTANCE_THRESHOLD = 15 

-- Auxiliary function for conditional teleport
...
Farm:AddToggle({
    ...
    Callback = function(v)
        ...
        -- Full reset of control variables to avoid conflicts
        _G.Level = false  
        _G.AutoFarm_Bone = false  
        _G.AutoFarm_Cake = false  
        _G.AutoTyrant = false 
...
-- Settings
local FarmHeight = 45 -- Safe height to not bug into the mob

-- Reinforced NoClip (To avoid getting stuck on walls/mobs)
spawn(function()
...
-- Magic Function: Selects the exact Mob/Quest by Level
...
-- Checks how many "Eyes" (Vases) were broken
...
-- Uses skills to break the vases
...
spawn(function()
    local VaseIndex = 1 -- Index to control which vase to attack
    
    while task.wait() do
        -- Only runs if the toggle is activated AND the mode is Tyrant
        if _G.AutoTyrant and _G.StartFarm then
            pcall(function()
                local plr = game.Players.LocalPlayer
                ...
                -- [PRIORITY 2] BREAK 4 VASES TO SUMMON
                elseif eyes == 4 then
                    local targetsty = {
                    ...
                    -- FIX: Uses VaseIndex to cycle between vases instead of resetting the loop
                ...
                -- [PRIORITY 3] NORMAL FARM (ALL MOBS + QUEST)
                else
        ...
        if _G.AutoFarm_Bone and _G.StartFarm then
            pcall(function()
                ...
                if _G.AcceptQuest and not QuestUI.Visible then      
                    ...
                    if distToNPC > 5 then
                        _tp(npcPos) -- Teleports directly to the NPC
                    else
                        -- If close, try to take the quest
                        task.wait(0.5)
                        local quests = {
                ...
                -- [STEP 2] IF ALREADY HAS QUEST OR DOES NOT WANT TO ACCEPT, GO FARM
                else
                    ...
                    if mob then      
                        -- Teleports to the mob and kills
                        _tp(mob.PrimaryPart.CFrame)      
                        EquipWeapon(_G.SelectWeapon)
                        G.Kill(mob, true)
                    else      
                        -- If no mobs found, go to their spawn spot
...
-- Maximum Distance Configuration (in studs)
-- Increase if you want to grab mobs a bit farther, decrease if you want them very close.
...
Farm:AddToggle({
    ...
    Description = "",
    -- 1. Loads if it was on or off when opening the script
    Default = GetSetting("AutoFarmNear_Save", false),
    Callback = function(I)
        _G.AutoFarmNear = I
        
        -- 2. Saves to the settings table for Save
        _G.SaveData["AutoFarmNear_Save"] = I
        
        -- 3. Saves to Settings.json file na pasta do executor
        SaveSettings()
    end,
})
...
Farm:AddToggle({
    ...
    Description = "",
    -- 1. Tries to load if it was on or off
    ...
    while task.wait(Sec) do
        pcall(function()
            ...
            if Core and Core:FindFirstChild("Humanoid") and Core.Humanoid.Health > 0 then
                ...
                -- Core not found → goes to the middle of the factory
                _tp(FactoryPos)
            end
        end)
    end
...
Farm:AddToggle({
    ...
    Callback = function(I)
        ...
        -- 3. Saves to Settings.json file automaticamente
        SaveSettings()
    end,
})
...
Farm:AddToggle({
    ...
    Description = "",
    -- 1. Loads the saved state (or false by default)
    Default = GetSetting("AutoFarmChest_Save", false),
    Callback = function(I)
        _G.AutoFarmChest = I
        
        -- 2. Saves to the save table
...
Farm:AddToggle({
	Name = "Auto Farm",
	Description = "",
	-- 1. Loads if the material farm was enabled
...
Farm:AddToggle({
    ...
    Description = "",
    -- 1. Loads if it was enabled or not
...
Setting:AddButton({
    ...
    Callback = function()
        -- Checks if the function exists before calling
        if SaveSettings then
            SaveSettings()
            
            -- Universal Notification (Works without Fluent lib)
            ...
            warn("Error: SaveSettings function not found!")
        end
    end
})
...
Setting:AddButton({
    Name = "Reset Settings",
    Description = "Deletes the save file and reverts to default",
    Callback = function()
        ...
        if isfile and isfile(FullPath) then
            ...
            game.StarterGui:SetCore("SendNotification", {
                Title = "Abacaxi Hub",
                Text = "Settings reset! Re-execute the script.",
                Duration = 5
            })
        else
            game.StarterGui:SetCore("SendNotification", {
...
Setting:AddToggle({
	Name = "Auto Attack",
	Description = "",
	-- 1. Loads the saved state or starts as true (original default)
...
Setting:AddToggle({
	Name = "Bring Mobs",
	Description = "",
	-- 1. Loads the saved state or starts as true
...
-- Settings Iniciais Globais (serão sobrescritas pelo Save se existirem)
    ...
    while task.wait(0.5) do
        if _G.AutoFishing then
            pcall(function()
                ...
                -- Checks if the selected rod is equipped
                if _G.SelectedRod and (not equippedTool or equippedTool.Name ~= _G.SelectedRod) then
                    ...
                    if rodInBag then
                        ...
                        return -- Stops execution here to avoid error trying to fish without a rod
                    end
                end
...
Others:AddToggle({
    ...
    Description = "Takes quest with the Angler NPC",
    -- Loads if it was enabled
...
Others:AddToggle({
    ...
    Description = "Spams the Z skill of the equipped tool",
    -- Loads if it was enabled
...
spawn(function()
 ...
 Others:AddToggle({
    ...
    Description = "",
    -- 1. Loads the saved state or starts as true (your script default)
...
local z5 = {
    ...
    -- Danger Level Configuration (SEA 3 ONLY)
    if World3 then
        local H5 = {
...
spawn(function()
    while wait() do
        if _G.SailBoats then
            pcall(function()
                local I = CheckBoat();
                -- FIX: Buys boat even if there are enemies, to not get stuck at spawn
                if not I then
    ...
    if _G.SelectedSkills[category] then
        ...
        for _, skill in ipairs(keys) do
            if _G.SelectedSkills[category][skill] then
                ...
                task.wait(0.35) -- Wait a bit between each skill to not bug
            end
        end
    end
end
...
Event:AddToggle({
    ...
    -- 1. Loads if the teleport was active
    Default = GetSetting("FrozenTP_Save", false),
    Callback = function(I)
        _G.FrozenTP = I
        
        -- 2. Saves to save memory
...
Event:AddToggle({
    ...
    -- 1. Loads if it was previously activated
...
Event:AddToggle({
    ...
    Description = "",
    -- 1. Loads if the automatic trade was enabled
...
Fruit:AddToggle({
    ...
    Description = "Starts the Raid automatically and waits 10s between checks",
    Default = false,
    Callback = function(I)
        _G.Auto_StartRaid = I;
    end,
...
spawn(function()
    -- The loop now waits 10 seconds for each execution
    while true do 
        task.wait(10)
        
        if _G.Auto_StartRaid then
            pcall(function()
                -- Only tries to start if the Raid is not active (Timer invisible)
                if plr.PlayerGui.Main.TopHUDList.RaidTimer.Visible == false then
                    
                    -- Checks if the player has the Microchip
                    if GetBP("Special Microchip") then
                        ...
                        elseif World3 then
                            -- 1. Tries to use the portal via InvokeServer (only a quick attempt)
                            replicated.Remotes.CommF_:InvokeServer("requestEntrance", Vector3.new(-5097.93164, 316.447021, -3142.66602));
                            
                            -- Small pause for the game teleport to happen
                            task.wait(0.5); 
                            
                            -- 2. Fallback: If the portal failed or the player does not have access, 
                            -- the script uses your _tp to go to the button position.
                            _tp(CFrame.new(-5033.50879, 315.014252, -2947.77539));
                            
                            -- 3. Triggers the Raid button
...
-- Attack enemies using YOUR G.Kill
...
while wait(Sec) do
    if _G.Raiding then
        ...
        if plr.PlayerGui.Main.TopHUDList.RaidTimer.Visible == true then
            ...
            -- Get the correct island
            local nextIsland = getNextIsland()
            if nextIsland then
                -- USES YOUR REAL TELEPORT
...
Fruit:AddToggle({
    ...
    -- 1. Loads if the automatic roll was enabled
...
Fruit:AddToggle({
    ...
    -- 1. Loads if automatic storage was enabled
...
Setting:AddButton({
    Name = "🚀 Ultra Optimize (All)",
    Description = "Run everything: remove effects, clean RAM, reduce render, disable shadow",
    Callback = function()
        LowCpu()
        NotificacaoNightMystic("nyann os", "✅ Ultra Optimizer activated!")
    end
});

Setting:AddButton({
    Name = "🧹 Clean RAM Now",
    Description = "Clear junk, dead enemies, collectgarbage",
    Callback = function()
        pcall(function()
            -- Remove dead enemies
            for _, enemy in ipairs(workspace.Enemies:GetChildren()) do
        ...
        pcall(function() collectgarbage("collect") end)
        pcall(function() collectgarbage() end)
        NotificacaoNightMystic("nyann os", "🧹 RAM cleaned!")
    end
});

Setting:AddButton({
    Name = "✨ Remove NPC Effects",
    Description = "Disable particles, fire, smoke, trail on Enemies",
    Callback = function()
        pcall(function()
        ...
        NotificacaoNightMystic("nyann os", "✨ NPC effects removed!")
    end
});

Setting:AddButton({
    Name = "🌑 Disable Map Shadows",
    Description = "Chunked — disable CastShadow on Map, strongly reduce GPU",
    Callback = function()
        task.spawn(function()
            pcall(function()
                local map = workspace:FindFirstChild("Map")
        ...
        NotificacaoNightMystic("nyann os", "🌑 Disabling map shadows...")
    end
});

Setting:AddButton({
    Name = "💡 Disable Post-Processing (Lighting)",
    Description = "Remove Blur, Bloom, SunRays, ColorCorrection, Fog, Atmosphere",
    Callback = function()
        pcall(function()
        ...
        NotificacaoNightMystic("nyann os", "💡 Post-processing removed!")
    end
});

Setting:AddButton({
    Name = "🎮 Render Quality Level 1",
    Description = "Force render quality to lowest",
    Callback = function()
        pcall(function() settings().Rendering.QualityLevel = "Level01" end)
        pcall(function()
            UserSettings():GetService("UserGameSettings").SavedQualityLevel = Enum.SavedQualitySetting.QualityLevel01
        end)
        NotificacaoNightMystic("nyann os", "🎮 Render Quality → Level 1!")
    end
});

Setting:AddToggle({
    Name = "🔄 Auto Clean RAM (every 30s)",
    Description = "Automatically clean junk + dead enemies every 30 seconds",
    Default = false,
    Callback = function(val)
        ...
        if val then
            ...
            NotificacaoNightMystic("nyann os", "🔄 Auto RAM Clean: ON")
        else
            NotificacaoNightMystic("nyann os", "🔄 Auto RAM Clean: OFF")
        end
    end
});
...
Setting:AddButton({
    ...
    Description = "Compress vertical screen to increase FPS",
    Callback = function()
        ...
        NotificacaoNightMystic("nyann os", "📺 Stretch Screen enabled!")
    end
});
...
local function SafeWaitForChild(parent, childName)
    ...
    if not success or not result then
        warn("SafeWaitForChild failed: " .. tostring(childName))
    end
    return result
end
...
NotificacaoNightMystic("nyann os", "👋 Welcome to nyann os (Freemium)")
