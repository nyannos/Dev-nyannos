...
local function NotificacaoNightMystic(titulo, mensagem)
    ...
    if not success then
        warn("[nyann os] Erro ao exibir notificação")
    end
end
...
-- RAID STATUS
-- ========================================
v10:AddSection({"Auto Raid (Quantum)"})

v10:AddButton({
    Name = "Auto Raid",
    Callback = function()
        -- Xài đỡ Quantum vì chưa có code riêng — bấm vào là hiện menu Quantum luôn
        ...
        if ok then
            pcall(function()
                redzlib:Notify({
                    Title = "Auto Raid",
                    Message = "Đã mở Quantum Onyx — dùng Auto Raid trong menu đó",
                    Duration = 4
                })
            end)
        else
            pcall(function()
                redzlib:Notify({
                    Title = "Auto Raid",
                    Message = "Lỗi load Quantum: " .. tostring(err),
                    Duration = 5
                })
            end)
            warn("[nyann os] Quantum Onyx load error:", err)
        end
    end
})
