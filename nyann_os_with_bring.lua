...
--==================================================
-- BRING SYSTEM (Aggressive Stack - BodyVelocity)
--==================================================
_G = _G or {}
_B = true  -- always allow when called from Kill
PosMon = PosMon or nil
_G.BringRange = _G.BringRange or 350
_G.MaxBringMobs = _G.MaxBringMobs or 25

local function IsRaidMob(mob)
    if not mob then return true end
    local n = (mob.Name or ""):lower()
    if n:find("raid") or n:find("microchip") or n:find("island") or n:find("boat") then
        return true
    end
...
BringEnemy = function(Mon)
    ...
    local maxMobs = _G.MaxBringMobs or 25
    ...
    for _, v in ipairs(workspace.Enemies:GetChildren()) do
        ...
        -- Same type if Mon given, otherwise any mob
        if mainName and v.Name ~= mainName then continue end
        ...
        -- Force position if network owner
        if Network(root) or dist > 8 then
    ...
    if Mon then
        ...
        if mRoot then
            ...
            local bv = mRoot:FindFirstChild("BringBV")
            if not bv then
                bv = Instance.new("BodyVelocity")
                bv.Name = "BringBV"
                bv.MaxForce = Vector3.new(1e9, 1e9, 1e9)
...
-- Continuous bring loop (keeps stacking while farming)
task.spawn(function()
