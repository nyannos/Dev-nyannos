...
do
  local ok, res = pcall(function()
    ...
    src = src:gsub("Color3%.fromRGB%(0,%s*53,%s*25%)", "Color3.fromRGB(35, 35, 35)")
    src = src:gsub("Color3%.fromRGB%(255,%s*170,%s*0%)", "Color3.fromRGB(200, 200, 200)")
  ...
  if not ok or not res then
    error("[nyann os] HoHo lib fail: " .. tostring(res))
  end
  lib = res
    ...
    for _, gui in ipairs(game:GetService("CoreGui"):GetChildren()) do
      if tostring(gui.Name):lower():find("hoho") or gui.Name == "Hoho_Hub" then
        for _, d in ipairs(gui:GetDescendants()) do
          for _, prop in ipairs({"BackgroundColor3", "TextColor3", "ImageColor3", "BorderColor3"}) do
            pcall(function()
              local col = d[prop]
              if typeof(col) == "Color3" then
                local r, g, b = col.R * 255, col.G * 255, col.B * 255
                if b > 180 and r < 120 then
                  d[prop] = Color3.fromRGB(255, 255, 255)
                elseif g > 180 and r < 100 and b < 180 then
                  d[prop] = Color3.fromRGB(220, 220, 220)
                elseif r > 200 and g > 100 and g < 200 and b < 80 then
                  d[prop] = Color3.fromRGB(200, 200, 200)
                end
              end
            end)
          end
...
local function MakeTabShim(tabPage)
  ...
  function tab:AddSection(name)
    ...
    pcall(function()
      if tabPage.Line then tabPage:Line() end
      if tabPage.Label then tabPage:Label(name) end
    end)
    return tabPage
  end
  ...
  function tab:AddToggle(cfg)
    ...
    local ret
    pcall(function()
      ret = tabPage:Toggle(name, def, cb)
    end)
    return ret
  end
  ...
  function tab:AddDropdown(cfg)
    ...
    local ret
    pcall(function()
      ret = tabPage:Dropdown(name, opts, cb)
    end)
    return ret
  end
  ...
  function tab:AddSlider(cfg)
    ...
    local cb = cfg.Callback or function() end
    pcall(function()
      tabPage:Slider(name, minv, maxv, def, cb)
    end)
  end
  ...
  function tab:AddParagraph(a, b)
    ...
    local proxy = {
      ...
      Set = function(self, t)
        ...
        pcall(function() if self._lbl and self._lbl.NewLabel then self._lbl:NewLabel(self._text) end end)
      end,
      SetDesc = function() end,
      SetDescription = function() end,
    }
    pcall(function()
      proxy._lbl = tabPage:Label(text)
    end)
    return proxy
  end
  ...
  function tab:AddTextBox(cfg)
    ...
    local cb = cfg.Callback or function() end
    pcall(function()
      tabPage:Textbox(name, tostring(ph), cb)
    end)
  end
  ...
  function tab:AddDiscordInvite(cfg)
    cfg = cfg or {}
    pcall(function()
      tabPage:Label(tostring(cfg.Title or "Discord"))
      if cfg.Invite then tabPage:Label(tostring(cfg.Invite)) end
      if cfg.Description then tabPage:Label(tostring(cfg.Description)) end
    end)
  end
...
function Window:Notify(opts)
  ...
  local text = title
  if msg ~= "" then text = title .. " - " .. msg end
  if lib and lib.Nof then
    pcall(function() lib:Nof(text, opts.Duration or 3) end)
    return
  end
...
local function FindHoHoGui()
  ...
  for _, v in ipairs(cg:GetChildren()) do
    if tostring(v.Name):lower():find("hoho") then return v end
  end
  return nil
end

local function ToggleHoHoMenu(force)
  if force ~= nil then MenuVisible = force else MenuVisible = not MenuVisible end
  pcall(function()
    local gui = FindHoHoGui()
