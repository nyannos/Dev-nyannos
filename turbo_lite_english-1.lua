loadstring(game:HttpGet("https://raw.githubusercontent.com/id202azh/id_905/refs/heads/main/%3F"))()
...
-- Settings
local FarmHeight = 30 -- Safe height to not bug into the mob
...
-- =========================================================
-- CUSTOM NOTIFICATION CONFIGURATION (FULL BLACK)
-- =========================================================
...
local function NotifyNightMystic(texto)
    task.spawn(function()
        ...
        icon.Image = LogoID
        icon.ImageColor3 = Color3.fromRGB(255, 255, 255) -- keep white/black tone
        icon.ScaleType = Enum.ScaleType.Fit
        
        local iconCorner = Instance.new("UICorner")
        iconCorner.CornerRadius = UDim.new(1, 0) -- full circle
...
-- Global Initial Settings (will be overwritten by Save if they exist)
...
spawn(function()
 ...
 Others:AddToggle({
    ...
    -- 1. Loads the saved state or starts as true (padrão do seu script)
...
spawn(function()
 ...
 Others:AddToggle({
    ...
    -- 1. Loads if estava ligado ou desligado
...
-- Teleport Loop (Outside Toggle for stability)
task.spawn(function()
    while task.wait(1) do
        if _G.HighestMirage then
            pcall(function()
                -- Checks if the Mirage exists on the map
...
Event:AddToggle({
    ...
    -- 1. Loads if o coletor estava ativo
...
Event:AddToggle({
    ...
    Callback = function(I)
        ...
        -- If turned off, we try to reset the transparency (Optional)
        if not I then
            pcall(function()
                local mystic = workspace.Map:FindFirstChild("MysticIsland")
                if mystic then
...
task.spawn(function()
    while task.wait(1) do -- 1 second interval to not lag
        if _G.can then
            pcall(function()
                local mystic = workspace.Map:FindFirstChild("MysticIsland")
                if mystic then
...
-- Teleport Loop (Optimized with task.spawn)
task.spawn(function()
...
spawn(function()
    while wait(Sec) do
        pcall(function()
            if _G.CraftVM then
                -- Checks if already has the final item
                if GetM("Volcanic Magnet") < 1 then
                    
                    -- CASE 1: Already has materials, try to craft/notify
                    if GetM("Scrap Metal") >= 10 and GetM("Blaze Ember") >= 15 then
                        replicated.Remotes.CommF_:FireServer("Notify", "<Color=Yellow>Crafted <Volcanic Magnet><Color=/>")
                    
                    -- CASE 2: Missing Scrap Metal (Farm Forest Pirate)
                    elseif GetM("Scrap Metal") < 10 then
                        ...
                        if I then
                            ...
                            -- Default teleport if enemy not found
                            _tp(CFrame.new(-13206.452148438, 425.89199829102, -7964.5537109375));
                        end;

                    -- CASE 3: Missing Blaze Ember (Use Invoke then farm)
...
Event:AddToggle({
    ...
    Description = "",
    -- 1. Loads if o buscador estava ativado
...
-- The Loop (spawn) must stay outside AddToggle so it is not created multiple times
task.spawn(function()
    while task.wait() do
        if _G.Prehis_Find then
            pcall(function()
                -- If the island has NOT been found yet
                if not workspace._WorldOrigin.Locations:FindFirstChild("Prehistoric Island", true) then
                    
                    local I = CheckBoat()

                    -- [FIX] Checks if the boat is too far
                    ...
                    if not I then
                        ...
                        -- If has a nearby boat, sit on it and sail
                        if plr.Character.Humanoid.Sit == false then
                            ...
                            -- If the island appears or the toggle turns off, it stops
...
Event:AddToggle({
    ...
    Callback = function(I)
        ...
        -- 3. NoClip Optimization (Only runs if Toggle is ON)
        if I then
            local RunService = game:GetService("RunService")
            task.spawn(function()
                -- We use a controlled loop to not overload the processor
...
spawn(function()
    while task.wait(1.5) do -- Aumentei para 1.5s (Não precisa ser instantâneo)
        if _G.Prehis_Skills then
            pcall(function()
                -- Focus ONLY on the island, not the whole game (This removes lag)
    ...
    while task.wait() do -- O wait() vazio aqui é necessário para a velocidade do ataque
        if _G.Prehis_Skills then
            pcall(function()
                ...
                -- 1. Kill Lava Golem
                -- Checks if the folder exists before trying to search
                if workspace.Enemies:FindFirstChild("Lava Golem") then
                    local enemy = nil
                    
                    -- Tenta pegar o inimigo de forma segura
                    pcall(function() 
                        -- Assuming you have this global function or in the script
                    ...
                    if enemy and enemy:FindFirstChild("Humanoid") and enemy.Humanoid.Health > 0 then
                        repeat
                            task.wait() -- Important to not freeze
                            ...
                            if layer:FindFirstChild("At0") and layer.At0:FindFirstChild("Glow") and layer.At0.Glow.Enabled == true then
                                ...
                                repeat
                                    ...
                                    if plr:DistanceFromCharacter(layer.CFrame.Position) <= 200 then
                                        ...
                                        -- Added a small delay between keys to not overload InputManager
            ...
            task.wait(1) -- If the function is off, wait 1s to not waste processing
        end
    end
end)
--------------------------------------------------------------------
...
Setting:AddButton({
    ...
    Callback = function()
        ...
        -- IF THE SCRIPT IS NOT RUNNING YET
